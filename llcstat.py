...
BPF_HASH(store_count, struct key_t);
...
int on_store_op(struct bpf_perf_event_data *ctx) {
    struct key_t key = {};
    get_key(&key);
    store_count.increment(key, ctx->sample_period);
    return 0;
}
"""
...
try:
    ...
    b.attach_perf_event(
        ev_type=PerfType.HARDWARE, ev_config=PerfHWConfig.BRANCH_INSTRUCTIONS,
        fn_name="on_store_op", sample_period=args.sample_period)
except Exception:
...
store_count = {}
for (k, v) in b.get_table('store_count').items():
    print("k: " + str(k.pid) + ", " + str(k.cpu) + ", " + str(k.name) + ", " + ", v: " + str(v.value))
